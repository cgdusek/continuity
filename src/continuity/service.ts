...
function getLegacyScopeFallback(params: {
  ...
  if (isDirectSourceClass(params.sourceClass) && params.config.identity.mode === "single_user") {
    return {
...
export class ContinuityService {
  ...
  private shouldCaptureRecent(params: {
    ...
    return (
      ...
      isDirectSourceClass(params.sourceClass) &&
      params.scope.scopeKind === "subject" &&
  ...
  async captureTurn(
    ...
    return this.lock(async () => {
      ...
      if (this.shouldCaptureRecent({ config: pluginConfig, sourceClass, scope })) {
